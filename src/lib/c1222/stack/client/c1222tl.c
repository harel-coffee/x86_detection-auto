            ...
            Unsigned8* destinationNativeAddress, Unsigned16 addressLength)
{
    ...
    messageBuffer[0] = 0x74; // This indicates that it is a send message command.
...
void C1222TL_SendNegotiateRequest(C1222TL* p)  // params are all properties of datalink layer
{
	...
	printf("CM: Sent a Negotiate request\n");
}
#endif // C1222_INCLUDE_COMMMODULE
    ...
    while ( p->status.receivedMessageIsComplete )
    {
        ...
        if ( p->rxMessageLength > 0 )
        {
            ...
            if ( command <= 0x1f ){   // this is a response{
				//if(p->status.isCommModule)
					//printf("CM : TL In Handle Received Transport Message. Received a response Message\n");
				//else
					//printf("Reg : TL In Handle Received Transport Message. Received a response Message\n");
                ProcessResponseMessage(p, command);
            }
			else{
				//if(p->status.isCommModule)
					//printf("CM : TL In Handle Received Transport Message. Received a command Message\n");
				//else
					//printf("Reg : TL In Handle Received Transport Message. Received a command Message\n");
...
static void ProcessReceivedNegotiateRequest(C1222TL* p)
{
    ...
    printf("Reg: Received a Negotiate request\n");
    p->counters.negotiateAttempted++;
...
static void ProcessReceivedNegotiateResponse(C1222TL* p)
{
    ...
    Unsigned32 negotiatedBaud = 9600;
	printf("CM: Received a Negotiate response\n");
...
static void ProcessReceivedGetConfigRequest(C1222TL* p)
{
    ...
    C1222AL_ConstructCallingApTitle(pApplication, &myApTitle); // Assigns the aptitle in transportconfig/registrtion status of the Application to myAptitle
    
    // convert my aptitle to relative for use in the comm module
    
    C1222ApTitle_Construct(&myRelativeAptitle, myRelativeAptitleBuffer, C1222_APTITLE_LENGTH); // Assigns the buffer to myRelativeAptitle
...
static void ProcessReceivedSendMessageRequest(C1222TL* p)
{
	...
	//if(p->status.isCommModule)
	//	printf("CM : TL Received a Send Message Request\n");
	//else
	//	printf("Reg : TL Received a Send Message Request\n");
...
static void ProcessReceivedGetStatusResponse(C1222TL* p)
{
    ...
    { 
		...
		printf("Reg : Got Get Status Response from CM. Updating Info\n");
