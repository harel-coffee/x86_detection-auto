...
void RegisterThread(void* param)
{
    ...
    initTables(); // What is this ? *******

    #ifdef ENABLE_LOCAL_PORT_ROUTER
	C1222Stack_Init(&registerStack, &registerLocalPortRouter, g_myPort, FALSE, 0, FALSE, FALSE);
    #else
    C1222Stack_Init(&registerStack, 0, g_myPort, FALSE, 0, FALSE, FALSE); //G_myPort = 6, is not Commmodule,localport or relay *
	#endif // ENABLE_LOCAL_PORT_ROUTER
    ...
    while(1)
    {
    	...
    	C1222Stack_Run(&registerStack); // Runs the device. It is initially in IDLE state
