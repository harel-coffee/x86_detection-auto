...
static C1219TableServer registerTableServer;
...
static C1222Server registerServer;
...
void RegisterThread(void* param)
{
    ...
    while(1)
    {
        ...
        {
            printf("Reg : Received a message from Client. have to process it \n");
...
void CommModuleThread(void* param)
{
    ...
    while(1)
    {
        ...
        if ( !messageSent && C1222Stack_IsMessageAvailable(&commModuleStack) )
        {
            ...
            {
                // send the message to the client. These messages should ideally come from the register thread.

#ifdef WANT_TCPIP
                /*localBuffer[0] = 0x60;
                length = C1222Misc_EncodeLength(pMsg->length, &localBuffer[1]);

                bytesSent = StartSendToClient(localBuffer, (Unsigned16)(length+1)); // It is sent through the TCP port. *
                if ( pMsg->length > 0 )
				...
				printf("CM : Sending Message to Client. Called Send to Client. \n");
				
				temp = SendToClient(localBuffer, (Unsigned16)(pMsg->length+2)); 
				//temp = 1;
				if(temp == -1)
				{
					bytesSent = 0;
					printf("CM : Couldn't send data. Encountered error in TcpIpSend_to_Client\n");
				}
				else
					bytesSent = temp;
#endif
        ...
        // check for and process messages from the client (eg, tables test bench which is a software from trilliant) or from my Client
#ifdef WANT_TCPIP
        messageSent = HandleMessageFromClient(&commModuleStack); //Sends messages after validation, to their intended native address which is obtained from the stored list
