...
Boolean C1222AL_GetMyApTitle(C1222AL* p, C1222ApTitle* pApTitle)
{
    ...
    return C1222ApTitle_TableValidate(pApTitle); // check whether the Aptitle has already been assigned
}
...
// Imcrement the statistic with the given id by 1
...
// Add the amount "count" to the statistic with the id = "id"
...
// set the lower 32 bits of the statistic with id = "id" 
...
// buffer is 6 bytes
// what ever is in the buffer is copied into the statistic with id = "id"
...
void C1222AL_ConstructCallingApTitle(C1222AL* pAppLayer, C1222ApTitle* pApTitle)
{
    ...
    {// If not registered this config will be used.
        C1222ApTitle_Construct( pApTitle, pAppLayer->transportConfig.nodeApTitle, C1222_APTITLE_LENGTH); // Assigned pAppLayer->transportConfig.nodeApTitle to pApTitle;
    }
...
// Update the average time required to process messages.
...
{  
    ...
    if ( p->status.sendingRegistrationRequest )
        ...
        C1222AL_LogEvent(p, C1222EVENT_AL_NOTERECEIVEMESSAGEHANDLINGCOMPLETE); // if it is not a registration request *
    ...
    if ( p->status.isReceivedMessageResponsePending ) // this is the normal case
    {
        ...
        NoteReceivedCompleteMessageHandlingComplete(p); // Update the avg time
        
        if ( !p->status.sendingRegistrationRequest ) // it was generated here
            C1222TL_SendPendingSendMessageOKResponse((C1222TL*)(p->pTransport)); // signals the transport layer
...
// Returns 0 ( stack is not waiting) if received message handling is complete or if received message response is not pending or both. *
...
{    
    ...
    if ( p->status.isReceivedMessageResponsePending ) // this is the normal case
    {
        ...
        NoteReceivedCompleteMessageHandlingComplete(p); // Upddate the avg time *
        
        if ( !p->status.sendingRegistrationRequest ) // it was generated here
            C1222TL_SendPendingSendMessageFailedResponse((C1222TL*)(p->pTransport), result); // Signal to the transport layer of the failure *
...
// Returns whether the message handling is complete or not *
...
// Just received the complete message. Response has not been sent yet. So the message handling is not yet complete. Update the time at which the message was received. It 
// is later used to update avg time *
...
// Check if the transport layer is busy and then run it when it is not. *
...
// Make a log entry containing the aptitle, message type ... *
...
// Checks whether the message is intended for the Comm module *
Boolean C1222AL_IsMsgForCommModule(C1222AL* p, ACSE_Message* pMsg)
{
    ...
    if ( ! ACSE_Message_GetCalledApTitle(pMsg, &calledApTitle) ) // Assigns the extracted called aptitle to the variable calledAptitle *
        return TRUE;  // not sure of this
    
    if ( C1222AL_GetMyApTitle(p, &myApTitle) ) // Assigns the extracted aptitle to the variable myAptitle *
    {   
        // explicitly my aptitle
             
        if ( C1222ApTitle_IsApTitleMyCommModule(&calledApTitle, &myApTitle) ) // Checks if both are equal to determine the App ("myself" or comm module") which should process
																				// the message *
        {
            return TRUE;
        }
...
Boolean C1222AL_SendMessage(C1222AL* p, ACSE_Message* pMsg, Unsigned8* nativeAddress, Unsigned16 addressLength)
{
    ...
    if ( C1222TL_IsBusy(pTransport) ) // if transport layer is busy - Abort *
    {
		...
		printf("Transport layer is busy. Send message failed\n");
        return FALSE;
    }
    ...
    {
        #ifdef C1222_INCLUDE_COMMMODULE
        if ( CommModuleHandleNetworkMessageDuringRegistration(p, pMsg, addressLength, nativeAddress) ) // If it already registering
		// and it obtains another registration response, its info will be stored.
        {
            p->status.sending = FALSE;
            return TRUE;
        }
...
// ******
// It is a type of application like the comm module *
...
//Run the Application first by checking what the application is *
...
// Sets the arguments to the native address *
...
{    
	// Compute the random time after which registration shall be attempted again *
...
// This value is used in the random compute time for re attempting registration *
...
// These values are used in the random compute time for re attempting registration *
...
Boolean C1222AL_NeedToReregisterAfterConfigChange(C1222AL* p, C1222TransportConfig* pCfg)
{
    ...
    // check if current configuration differs from the proposed one. *
...
// If there is a need to register, change the configuration *
...
{    
    ...
    if ( memcmp(pCfg, &p->transportConfig, sizeof(C1222TransportConfig)) == 0 ) // if the config is already the same *
        return; // no change needed
    
    memcpy(&(p->transportConfig), pCfg, sizeof(C1222TransportConfig)); // else set the new config *
...
// Log that the configuration has been sent *
...
//************** These are the log functions that were earlier used *******
...
//************** Initialisation code : Set the Segmenter Links, receiver links, sender links, fragment size for sender and receiver, and registration retry values *
...
//****************** Initialisation of the AL , initialise sender and receiver, set configs to zero, set Aptitle, set Client Version, call common init code *
...
void C1222AL_MakeSimpleResponseMessage(C1222AL* p, ACSE_Message* pMsg, Unsigned8 response, Unsigned32 maxRequestOrResponse, ACSE_Message* pResponseMessage)
{
    ...
    ACSE_Message_SetBuffer( pResponseMessage, p->tempBuffer, sizeof(p->tempBuffer) ); // Set the buffer *
...
// Calls the prev function, constructs and sends the response *
void C1222AL_SimpleResponse(C1222AL* p, ACSE_Message* pMsg, Unsigned8 response, Unsigned32 maxRequestOrResponse)
{
    ...
    C1222AL_MakeSimpleResponseMessage(p, pMsg, response, maxRequestOrResponse, &responseMessage); // response message is constructed and stored in variable responseMesage *
    ...
    if ( C1222AL_GetReceivedMessageNativeAddress(p, &nativeAddressLength, &nativeAddress) ) 
    {
        ...
        C1222AL_SendMessage(p, &responseMessage, nativeAddress, nativeAddressLength); // Send the response message *
    }
    else
        C1222AL_NoteReceivedMessageHandlingIsComplete(p);
}
...
// similar to the previous function but the last else part is not present *
void C1222AL_SimpleResponseWithoutReceivedMessageHandling(C1222AL* p, ACSE_Message* pMsg, Unsigned8 response, Unsigned32 maxRequestOrResponse)
{
    ...
    if ( C1222AL_GetReceivedMessageNativeAddress(p, &nativeAddressLength, &nativeAddress) ) // Seems to always return true ?
    {
        // need to notify that the received message is complete
        //C1222AL_NoteReceivedMessageHandlingIsComplete(p);
...
ACSE_Message* C1222AL_MakeRegistrationMessage(C1222AL* p)
{
    ...
    // set the various fields of the message *
    request[0] = 0x27;
    // <node-type> ::= <byte>
...
ACSE_Message* C1222AL_MakeDeRegistrationMessage(C1222AL* p)
{
    ...
    request[0] = 0x24; // Probably code for Deregistration *
...
// Returns FALSE if the registration response is Invalid *
...
// Returns FALSE if the De-registration response is invalid *
...
// Just set some flags indicating that registration has to be done again *
...
// Ignore any other message other than a De-registration response in the middle of the De-registration process. *
